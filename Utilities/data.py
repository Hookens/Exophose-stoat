...
class Data(Gear):
    ...
    def _get_db_connection(self) -> MySQLConnection:
    ...
    async def get_server(self, server_id: str) -> ExoRole:
        ...
        return (
            ExoRole(id=result[0][0], server_id=result[0][1])
    ...
    async def get_badge_allowed_roles(self, server_id: str) -> list[AllowedRole]:
        result = await self._execute_read_operation(
            "ExoGetBadgeAllowedRoles", server_id
        )
        return (
            [
                AllowedRole(
    ...
    async def get_gradient_allowed_roles(self, server_id: str) -> list[AllowedRole]:
        result = await self._execute_read_operation(
            "ExoGetGradientAllowedRoles", server_id
        )
        return (
            [
                AllowedRole(
    ...
    async def get_member_roles(self, server_id: str, user_id: str) -> list[CreatedRole]:
        result = await self._execute_read_operation(
            "ExoGetMemberRoles", server_id, user_id
        )
        return (
            [
                CreatedRole(
    ...
    async def is_allowed_role(self, role_id: str) -> bool:
        return bool(
            (await self._execute_read_operation("ExoIsAllowedRole", role_id))[0][0]
        )

    async def is_member_role(self, role_id: str) -> bool:
        return bool(
            (await self._execute_read_operation("ExoIsMemberRole", role_id))[0][0]
        )

    async def count_member_roles(self, server_id: str, user_id: str) -> int:
        return int(
            (
                await self._execute_read_operation(
                    "ExoCountMemberRoles", server_id, user_id
                )
            )[0][0]
        )
    ...
    async def add_bundle_allowed_role(
        self, role_id: str, server_id: str, index: int
    ) -> bool:
        await self._log_sql_event(
            f"Bundle '{index}' allowed R'{role_id}' in S'{server_id}'", "INFO"
        )
        return await self._execute_write_operation(
            "ExoAddBundleAllowedRole", role_id, server_id, index
        )

    async def add_bundle_role(self, role_id: str, server_id: str, index: int) -> bool:
        await self._log_sql_event(
            f"Bundle '{index}' added R'{role_id}' in S'{server_id}'", "INFO"
        )
        return await self._execute_write_operation(
            "ExoAddBundleRole", role_id, server_id, index
        )
    ...
    async def get_bundle(self, server_id: str, index: int) -> Bundle:
        ...
        return (
            Bundle(id=result[0][0], server_id=result[0][1], name=result[0][2])
            if result is not None
            else None
        )
    ...
    async def get_bundles(self, server_id: str) -> list[Bundle]:
        ...
        return (
            [Bundle(id=row[0], server_id=row[1], name=row[2]) for row in result]
            if result is not None
            else None
        )
    ...
    async def get_bundle_allowed_roles(self, bundle_id: int) -> list[BundleRole]:
        result = await self._execute_read_operation(
            "ExoGetBundleAllowedRoles", bundle_id
        )
        return (
            [
                BundleRole(
    ...
    async def get_bundles_roles(self, server_id: str) -> list[int]:
        ...
        return [str(row[1]) for row in result] if result is not None else None

    async def get_bundles_choices(self, allowed_roles: list[BundleRole]) -> list[int]:
        if not allowed_roles:
            return []

        distinct_bundles: list[int] = list(role.bundle_id for role in allowed_roles)
        ...
        return list(role.id for role in bundle_roles)

    async def get_bundles_choice(
        self, allowed_roles: list[BundleRole], index: int
    ) -> int:
    ...
    async def get_allowed_bundle_roles(self, server_id: str) -> list[BundleRole]:
        result = await self._execute_read_operation(
            "ExoGetAllowedBundleRoles", server_id
        )
        return (
            [
                BundleRole(
    ...
    async def delete_bundle_allowed_role(
        self, role_id: str, server_id: str, index: int
    ) -> bool:
        await self._log_sql_event(
            f"Bundle '{index}' disallowed R'{role_id}' in S'{server_id}'", "INFO"
        )
        return await self._execute_write_operation(
            "ExoDeleteBundleAllowedRole", role_id, server_id, index
        )

    async def delete_bundle_role(
        self, role_id: str, server_id: str, index: int
    ) -> bool:
        await self._log_sql_event(
            f"Bundle '{index}' removed R'{role_id}' in S'{server_id}'", "INFO"
        )
        return await self._execute_write_operation(
            "ExoDeleteBundleRole", role_id, server_id, index
        )
    ...
    async def delete_bundles_allowed_role(self, role_id: str) -> bool:
        ...
        return await self._execute_write_operation(
            "ExoDeleteBundlesAllowedRole", role_id
        )
    ...
    async def is_bundle_allowed_role(self, role_id: str) -> bool:
        return bool(
            (await self._execute_read_operation("ExoIsBundleAllowedRole", role_id))[0][
                0
            ]
        )
    ...
    async def is_bundle_role(self, role_id: str) -> bool:
        return bool(
            (await self._execute_read_operation("ExoIsBundleRole", role_id))[0][0]
        )

    async def count_bundles(self, server_id: str) -> int:
        return int(
            (await self._execute_read_operation("ExoCountBundles", server_id))[0][0]
        )
